...
@if($workspace && $workspace->hasCompletedOnboarding() && !$workspace->hasPaymentConfigured() && $workspace->isCreator())
    <div class="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
        <div class="flex items-start">
            <flux:icon.exclamation-triangle class="w-6 h-6 text-amber-500 mt-0.5 flex-shrink-0" />
            <div class="ml-3">
                <flux:heading size="sm" class="text-amber-800">Payment Setup Required</flux:heading>
